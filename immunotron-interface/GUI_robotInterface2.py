...
class ExperimentHomePage(tk.Frame):
    def __init__(self,master):
        ...
        EMPTYTEXT = '-'
        NUMEXP = 4
        allLabels = ['Experiment:','Type (Author):','Name:','Incubator racks:','Fridge racks:','# plates:','Blank columns:','# timepoints:','Start Time:','Timepoints:','End Time:','Added to matrix:']
        
        #Load experiment parameters (experiments currently running on robot)
        if 'experimentParameters.pkl' not in os.listdir():
            ...
            self.allExperimentParameters = pickle.load(open('experimentParameters.pkl','rb'))
        #Load all saved experiment protocols
        if 'experimentProtocols.pkl' not in os.listdir():
            self.experimentProtocols = {}
            with open('experimentProtocols.pkl','wb') as f:
                pickle.dump(self.experimentProtocols,f)
        else:
            self.experimentProtocols = pickle.load(open('experimentProtocols.pkl','rb'))
        ...
        def updateExperimentLabels():
            ...
            for exp in range(NUMEXP):
                if len(self.allExperimentParameters[exp]) != 0:
                    ...
                    for i,expParameter in enumerate(['protocolParameters','experimentID','incubatorPositions','fridgePositions','numPlates','blankColumns','numTimepoints','startTime','timepointlist','addedToMatrix','addedToMatrix']):
                    ...
                    self.allExpInfoLabels[exp][-2].configure(text=endTime.strftime('%Y-%m-%d %a %I:%M %p'))
        ...
        def generateFullMatrix():
            ...
            for i,l in enumerate(self.allExpInfoLabels):
                ...
                if expName != EMPTYTEXT:
                    experimentProtocol = self.allExperimentParameters[i]['experimentProtocol']
                    experimentIDsToIntegrate.append(expName)
                    experimentsToIntegrate.append(i)
                    experimentProtocolsToIntegrate.append(experimentProtocol)
...
class ExperimentInfoPage(tk.Frame):
    def __init__(self,master,expNum):
        ...
        experimentProtocols = pickle.load(open('experimentProtocols.pkl','rb'))
        ...
        def collectInputs():
            ...
            incubator = pickle.load(open('incubatorStatus.pkl','rb'))
            fridge = pickle.load(open('fridgeStatus.pkl','rb'))
            ...
            experimentParameters['incubatorPositions'] = calculateIncubatorPositions(incubator, experimentProtocols[experimentProtocolVar.get()], int(plateNumberEntry.get()), int(timepointNumberEntry.get()))
            experimentParameters['fridgePositions'] = calculateFridgePositions(fridge, experimentProtocols[experimentProtocolVar.get()], int(plateNumberEntry.get()), [x+1 for x in range(12) if blankVarList[x].get()], int(timepointNumberEntry.get()))
